...
/* The arena the nodes live in, and one constructor per node type, are in
 * ast_build.h — included here so that `#include "cscript/ast.h"` still means
 * all of it. Split because the two answer different questions: the shapes
 * above are what a tree *is*, and the declarations below are how one is made.
 */
