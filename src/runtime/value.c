/* value.c — what a Value is, and what it converts to.
 *
 * Equality, truthiness, the type name a diagnostic prints, and the number
 * formatting — which is ECMA-262's `Number::toString` rather than printf's,
 * because `0.1 + 0.2` and `1e21` have to print the way JavaScript prints them
 * or the Node parity check fails on arithmetic that is otherwise identical.
 */
#include <math.h>
#include <stdio.h>
#include <stdlib.h>
...
int csValueFormatNumber(char *buffer, size_t size, double value) {
  return csValueFormatNumberEx(buffer, size, value, false);
}

void csValuePrint(Value value) {
  if (IS_NUMBER(value)) {
    char buffer[32];
    csValueFormatNumberEx(buffer, sizeof(buffer), AS_NUMBER(value), true);
    printf("%s", buffer);
